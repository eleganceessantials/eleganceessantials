...
export default function AdminDashboard() {
  ...
  const fetchData = async () => {
    try {
      ...
      setTotalProducts(
        typeof data.total === "number" ? data.total : safeProducts.length
      );
    } catch (error) {
      ...
      <div className="space-y-12 pb-10">
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            ...
            <p className="text-gray-500 font-medium mt-2 text-lg">
              Real-time overview of Elegance Essentials performance.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-purple-50 hover:shadow-xl transition-all group cursor-default">
            <div className="w-14 h-14 bg-purple-50 text-purple-600 rounded-2xl flex items-center justify-center text-2xl mb-6 group-hover:bg-purple-600 group-hover:text-white transition-all">
              📦
            </div>
            ...
            <p className="text-6xl font-[1000] text-gray-900 tracking-tighter">
              {loading ? "..." : totalProducts}
            </p>

            <p className="text-sm text-gray-500 mt-2 font-bold italic">
              Products currently live in your store
            </p>
          </div>
          ...
          <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-purple-50 hover:shadow-xl transition-all group cursor-default">
            ...
            <p className="text-4xl font-[1000] text-gray-900 tracking-tighter uppercase">
              {loading ? "Checking" : "Healthy"}
            </p>

            <p className="text-sm text-gray-500 mt-2 font-bold italic">
              MongoDB Cluster is synchronized
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 bg-gradient-to-br from-purple-600 to-indigo-800 rounded-[3rem] p-12 text-white relative overflow-hidden shadow-2xl">
            ...
            <div className="relative z-10 flex flex-col h-full justify-between gap-10">
              <div className="space-y-4">
                ...
                <p className="text-purple-100 font-medium text-lg max-w-md">
                  Your catalog is synchronized. Keep your inventory fresh with
                  new arrivals.
                </p>
              </div>
              ...
              <div className="flex flex-wrap gap-4">
                <Link
                  href="/admin/products"
                  className="bg-white text-purple-700 px-8 py-4 rounded-2xl font-black hover:bg-purple-50 transition-all shadow-xl active:scale-95 cursor-pointer"
                >
                  Manage Inventory →
                </Link>
                ...
                <Link
                  href="/"
                  className="bg-purple-500/30 backdrop-blur-md text-white border border-white/20 px-8 py-4 rounded-2xl font-black hover:bg-white/20 transition-all active:scale-95 cursor-pointer"
                >
                  View Live Site
                </Link>
              </div>
          ...
          <div className="bg-white p-10 rounded-[3rem] shadow-sm border border-purple-50 flex flex-col">
            <h3 className="text-xl font-[1000] text-gray-900 mb-8 tracking-tighter">
              Category Density
            </h3>
            ...
            <div className="space-y-6 flex-1">
                  ...
                  <div className="h-3 w-full bg-gray-50 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${
                          products.length > 0
                            ? Math.min((count / products.length) * 100, 100)
                            : 0
                        }%`,
                      }}
                    ></div>
              ...
              {categoryStats.length === 0 && (
                <p className="text-center text-gray-400 font-medium py-10 italic">
        ...
        <div className="flex items-center justify-between pt-6 border-t border-purple-50 px-4">
          <p className="text-xs font-black text-gray-300 uppercase tracking-[0.3em]">
            Elegance Essentials Security Shield Active
          </p>
